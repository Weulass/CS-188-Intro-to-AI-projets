class N_gram:
    ...
    def calc_word_probs(self):
        ...
        return self.normalize_dict(count)
        ### END YOUR CODE HERE ###
    ...
    def probs_to_neg_log_probs(self, probs_dict):
        ...
        """
        ### START YOUR CODE HERE ###
        probs_dict.update((key, -np.log(val)) for key, val in probs_dict.items())
        ### END YOUR CODE HERE ###
        return probs_dict
    ...
    def p_naive(self, curr_word, prev_phrase, n):
        ...
        try:
            ### START YOUR CODE HERE ###
            print('filtered_adj_counter = ', filtered_adj_counter)
            norm = self.calc_adj_probs(filtered_adj_counter)
            print("norm =", norm)
            phrase = list(prev_phrase)
            phrase.append(curr_word)
            prob = norm[tuple(phrase)]
            ### END YOUR CODE HERE ###
        except KeyError:
    ...
    def calc_prob_of_sentence(self, sentence_list, n, p_func=p_laplace):
        ...
        """
        ### START YOUR CODE HERE ###
        print(np.exp(self.calc_neg_log_prob_of_sentence(sentence_list, n)))
